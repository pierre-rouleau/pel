...
;; Time-stamp: <2026-09-10 14:24:26 EDT, updated by Pierre Rouleau>
...
(defun pel-treesit-remap-available-for (mode)
  "Return non-nil when treesit is available and the ts-MODE can be used.
...
NOTES:
-  :same-for-ts-early-remap is an internal option generated by
   `pel-setup-major-mode'; it should not be used directly.
- The macro generates code to set the behaviour of the tab key unless
  the major mode is identified inside the `pel--tab-controlling-major-modes'
...
Function created by the `pel-config-major-mode' macro."
    ...
    (when (and (eq ts-option :same-for-ts)
      ...
      ;; is available and working.  Therefore, when the tree-sitter mode
    ...
    (unless (eq ts-option :ts-only)
      (pel-append-to hook-body
        `((pel--mode-hook-maybe-call (function ,gn-fct1)
                                     (quote ,gn-mode-name)
                                     (quote ,gn-mode-hook)))))
    ;; 4.2 - Append support for ts-mode if necessary
