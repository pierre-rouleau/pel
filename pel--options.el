...
- The tree-sitter-langs package installs the language dynamic
  ...
  was done.
- The files stored inside that directory have a name like cmake.dylib,
  cmake.so or cmake.dll (depending on your OS extension for the dynamic
  libraries).
- These names do not follow Emacs naming convention for tree-sitter language
  grammar dynamic library files, which is: libtree-sitter-lang.ext .

Some some work is required to properly support tree-sitter.
...
- Create another directory: ~/.emacs.d/libtree-sitter-extra .
  Use this directory to store symbolic links to language grammar files
  dynamic library files that you prefer to use over the ones installed by
  the tree-sitter-langs package or files not yet included in the
...
Remember to update the tree-sitter-langs-bin symbolic link to the new version
of the ~/.emacs.d/elpa/tree-sitter-langs-YYYYMMDD.vvv/bin directory next time
you update the tree-sitter-langs package!"
  :group 'pel-pkg-for-tree-sitter
  :type '(repeat string))
...
and provides \\[smex-major-mode-commands] which does completion
commands related to active for major mode only.
...
(defcustom pel-with-cua-paste nil
  "Activate CUA-compliant Control-V: bind Control-V to yank.

When this is set to t, Control-V is bound to yank instead of the default."
  :group 'pel-pkg-for-cut-and-paste
  :type 'boolean
  :safe #'booleanp)
...
(defgroup pel-file-finding nil
  "Control file finding used by `pel-open-at-point' and other commands."
  :group 'pel-base-emacs
  :group 'pel-pkg-for-filemng)

;; Note: all user options of this group are defined as part of other groups
;;       to which they also belong.  The pel-file-finding group exists to
;;       simplify access to all the file finding control user-options across
...
(defcustom pel-use-helm-descbinds nil
  "Control whether PEL uses the package helm descbinds.
...
- The project settings (accessed with `pel-dev-project.settings'):
  ...
  - Project Used Tool(s):         The name(s) of tool(s) used by the project.
                    ...
                    Access it with: `pel-dev-project.setting.tools'
                                  ...
                                  a PEL_DEV_TOOLS_FOR_\\='LANG\\='
                                  (with \\='LANG\\=' being the language name
                                  such as C, CPP) that holds a colon-separated
                                  list of tool names.
...
- t                : use `vhdl-mode' provided by the '\\=vhdl-mode' external
                     package.
- with-tree-sitter : use `vhdl-ts-mode' provided by the '\\=vhdl-ts-mode'
                     external package."
...
\\[eldoc-mode], to toggle the `eldoc-mode')."
  :group 'pel-pkg-for-all-languages
  :type 'boolean
  :safe #'booleanp
...
When activating it you can select between the following values:
- t                : use `ada-mode' provided by the '\\=ada-mode' external
                     package.
- with-tree-sitter : use `ada-ts-mode' provided by the '\\=ada-ts-mode'
                     external package.
...
- with-tree-sitter : use `java-ts-mode' provided by the '\\=java-ts-mode'
                     built-in."
...
- js2-mode         : use `js2-mode' provided by the '\\=js2-mode' external
                     package,
...
When turned on, PEL sets `js2-mode-dev-mode-p' to t when '\\=js2-mode'
is loaded, to activate extra js2 commands and features."
  :group 'pel-pkg-for-javascript
  :type 'boolean
  :safe #'booleanp)
...
`erlang-root-dir' to the value identified by the variable
...
When activating it you can select between the following values:
- t                : use `python-mode' provided by the '\\=python-mode'
                     built-in library.
...
listed here.  It is located by searching for it using the project root
found by this mechanism, not by being listed as an anchor itself."
  :group 'pel-pkg-for-project-mng
  :group 'pel-file-finding
  :type '(repeat string))
...
It stops as soon as it finds one of these anchors, stopping the search
done for the other anchors identified by `pel-project-root-identifiers'."
  :group 'pel-pkg-for-project-mng
  :group 'pel-file-finding
  :type '(repeat string))
...
(defcustom pel-term-use-shell-prompt-line-regexp t
  "When t, set `term-prompt-regexp' to `pel-shell-prompt-line-regexp' value."
  :type 'boolean
  :safe #'booleanp)
